...
class RegistrationStepFourScreen extends StatefulWidget {
  ...
  @override
  State<RegistrationStepFourScreen> createState() => _RegistrationStepFourScreenState();
}

class _RegistrationStepFourScreenState extends State<RegistrationStepFourScreen> {
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Choose how to add your profile picture.', style: AppTextStyles.body),
            ...
            ListTile(
              ...
              title: Text('Choose from Gallery', style: AppTextStyles.subheading),
              subtitle: Text('Pick an existing photo', style: AppTextStyles.body),
              onTap: () async {
                Navigator.pop(ctx);
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text('Enter your card details to link to your patient wallet.', style: AppTextStyles.body),
              const SizedBox(height: 20),
              TextField(
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    if (cardNoCtrl.text.isEmpty || expiryCtrl.text.isEmpty || cvvCtrl.text.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Please fill out all fields'), backgroundColor: AppColors.statusBad),
                      );
                      return;
                    }
                    setState(() {
              ...
              children: [
                  ...
                  child: LiquidGlassCard(
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            const Icon(Icons.verified, size: 16, color: AppColors.statusGood),
                            const SizedBox(width: 6),
                            Text(
                              isPatient ? 'Verified Patient' : 'Verified Pharmacy',
                        ...
                        Center(
                          child: Text(
                            displayName.isNotEmpty ? displayName : 'James Mensah',
                        ...
                        Row(
                          ...
                          children: [
                            Row(
                              children: [
                                const Icon(Icons.check_circle_outline, size: 14, color: AppColors.statusGood),
                                const SizedBox(width: 4),
                                Text(
                                  'Verified',
                                  style: AppTextStyles.label.copyWith(color: AppColors.statusGood),
                                ),
                              ],
                            ),
                            Text(
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: _showCardInputDialog,
                    icon: Icon(_hasCardLinked ? Icons.credit_card : Icons.credit_card_outlined, size: 18),
                    label: Text(_hasCardLinked ? 'Payment Card Linked' : 'Add to Wallet'),
                    style: OutlinedButton.styleFrom(
                      foregroundColor: _hasCardLinked ? AppColors.statusGood : AppColors.textPrimary,
                      side: BorderSide(color: _hasCardLinked ? AppColors.statusGood : AppColors.hairline),
                ...
                SizedBox(
                  width: double.infinity,
                  child: ElevatedButton(
                    onPressed: state.submitStatus == RegistrationSubmitStatus.loading
                        ? null
                        : () async {
                            final cubit = context.read<RegistrationCubit>();
                            final success = await cubit.submitRegistrationToSupabase();
                            ...
                            if (success) {
                              ...
                              if (session != null) {
                                ...
                                context.go('/login', extra: {
                                  'expiredMessage': 'Account created! Check your email to confirm your address, then log in here.',
                                });
                              }
                            } else {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    cubit.state.submitError ?? 'Registration failed. Please try again.',
                                  ),
                                  backgroundColor: AppColors.statusBad,
                                ),
                              );
                    ...
                    child: state.submitStatus == RegistrationSubmitStatus.loading
                        ? const SizedBox(
                            height: 20,
                            width: 20,
